...
import java.util.Scanner;
    ...
    public class Job_Seeker extends User {
        ...
        public void setResumeGUI(int experience , String skills) {
            resume = new Resume( getName(), getEmail(), getAge(),experience,skills);
        }
        public void Browse(){
        ...
        public void removeGUI(Label lbl){
            String appl="";
            if(appliedJobs.size() == 0){
                lbl.setText("You have no applications right now!");
            } else {
                int i = 1;
                for(String appliedJob:appliedJobs){
                    appl=appl+i+"- "+appliedJob+"\n";
                    i++;
                }
                lbl.setText(appl);
        ...
        public void furtherRemoveGUI(TextField tf,Label lbl){
            int n = Integer.valueOf(tf.getText());
            if(n <= 0 || n > appliedJobs.size()){
                lbl.setText("Invalid option!");

            } else {
                Job.getJob(appliedJobs.get(n-1)).removeApplication(this);
                appliedJobs.remove(n-1);
                lbl.setText("Removed Successfully!");
            }
        }
